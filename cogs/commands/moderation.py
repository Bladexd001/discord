...
class Moderation(commands.Cog):
    ...
    async def unlockall(self,ctx, server: discord.Guild = None, *, reason=None):
        hacker = discord.Embed(color=0x01f5b6,description="<:GreenTick:1029990379623292938> | Unlocking all channels in few seconds .", timestamp=ctx.message.created_at)
        ...
        try:
            ...
            embed = discord.Embed(color=0x01f5b6,description=f"**Failed to unlock, {server}.**")
    ...
    async def lockall(self,ctx, server: discord.Guild = None, *, reason=None):
        hacker = discord.Embed(color=0x01f5b6,description="<:GreenTick:1029990379623292938> | Locking all channels in few seconds .", timestamp=ctx.message.created_at)
        hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")        
        await ctx.reply(embed=hacker)
        if server is None: server = ctx.guild
        try:
            for channel in server.channels:
                await channel.set_permissions(
                ctx.guild.default_role,
                overwrite=discord.PermissionOverwrite(send_messages=False, read_messages=True), reason=reason)
        except:
            embed = discord.Embed(color=0x01f5b6,description=f"**Failed to lockdown, {server}.**")
            embed.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")       
            await ctx.send(embed=embed)
        else:
            pass
    ...
    async def give(self,ctx, member: discord.Member, role: discord.Role,):
        ...
        if guild.me.top_role >= ctx.author.top_role:
              embed = discord.Embed(color=0x01f5b6,description="<a:error:1002226340516331571> | Your role is below the bot.")
              await ctx.send(embed=embed)
              return
        if member.top_role >= ctx.author.top_role:
            await ctx.send("**<a:error:1002226340516331571> | The provided user has roles that are above or have the same role as you.**")
            return
        else:   
            await member.add_roles(role)
            embed = discord.Embed(color=0x01f5b6)
            embed.set_author(name=f"Role Changed for {member.name}")
            embed.set_footer(text=f"Done by {ctx.author}",icon_url=f"{ctx.author.avatar}")
            embed.add_field(name="Added Role", value=f"{role} has been given to {member.name}")
            await ctx.send(embed=embed)
    ...
    async def _hideall(self,ctx):
        hacker = discord.Embed(color=0x01f5b6,description="<:GreenTick:1029990379623292938> | Hiding all channels in few seconds .", timestamp=ctx.message.created_at)
        hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
    ...
    async def _unhideall(self,ctx):
        hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Unhiding all channels in few seconds .", timestamp=ctx.message.created_at)
        hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        #hacker.set_thumbnail(url =f"{ctx.author.avatar}")
        await ctx.reply(embed=hacker)
        for x in ctx.guild.channels:
    ...
    async def _hide(self, ctx, channel: discord.TextChannel = None):
      ...
      hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Succefully Hidden {channel.mention} .", timestamp=ctx.message.created_at)
      hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
      hacker.set_thumbnail(url =f"{ctx.author.avatar}")
      await ctx.reply(embed=hacker)
    ...
    async def _unhide(self, ctx, channel: discord.TextChannel = None):
      ...
      hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Succefully Unhidden {channel.mention} .", timestamp=ctx.message.created_at)
      hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
      hacker.set_thumbnail(url =f"{ctx.author.avatar}")
      await ctx.reply(embed=hacker)

    @commands.hybrid_command(name="audit", help="See recents audit log action in the server .")
    @blacklist_check()   
    @ignore_check()
...
Reason: `{entry.reason}`\n\n''')
    ...
    async def _softban(self, ctx: commands.Context, member: discord.Member, *, reason=None):
        ...
        hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Sucessfully soft-banned {member}.", timestamp=ctx.message.created_at)
    ...
    @commands.hybrid_command(name="mute", description="Timeouts someone for specific time.",usage="mute <member> <time>",aliases=["timeout","stfu"])
    ...
    async def _mute(self, ctx, member: discord.Member, duration):
      ...
      till = duration[-1]
      if tame == -1:
        hacker3 = discord.Embed(color=0x01f5b6,description=f"<a:error:1002226340516331571> | You didnt didnt gave time with correct unit\nExamples:\n{ctx.prefix}mute{ctx.author} 10m\n{ctx.prefix}mute {ctx.author} 5hr", timestamp=ctx.message.created_at)
        await ctx.reply(embed=hacker3, mention_author=False)
      elif tame == -2:
        hacker4 = discord.Embed(color=0x01f5b6,description=f"<a:error:1002226340516331571> | Time must be an integer!", timestamp=ctx.message.created_at)
        await ctx.reply(embed=hacker4, mention_author=False)
      else:
        if till.lower() == "d":
          t = datetime.timedelta(seconds=tame)
          hacker = discord.Embed(color=0x01f5b6,description="<:GreenTick:1029990379623292938> | Successfully Muted {0.mention} For {1} Day(s)".format(member, ok), timestamp=ctx.message.created_at)
        elif till.lower() == "m":
          t = datetime.timedelta(seconds=tame)
          hacker = discord.Embed(color=0x01f5b6,description="<:GreenTick:1029990379623292938> | Successfully Muted {0.mention} For {1} Minute(s)".format(member, ok), timestamp=ctx.message.created_at)
        elif till.lower() == "s":
          t = datetime.timedelta(seconds=tame)
          hacker = discord.Embed(color=0x01f5b6,description="<:GreenTick:1029990379623292938> | Successfully Muted {0.mention} For {1} Second(s)".format(member, ok), timestamp=ctx.message.created_at)
        elif till.lower() == "h":
          t = datetime.timedelta(seconds=tame)
          hacker = discord.Embed(color=0x01f5b6,description="<:GreenTick:1029990379623292938> | Successfully Muted {0.mention} For {1} Hour(s)".format(member, ok), timestamp=ctx.message.created_at)
      try:
        if member.guild_permissions.administrator:
          hacker1 = discord.Embed(color=0x01f5b6,description="<a:error:1002226340516331571> | I can\'t mute administrators", timestamp=ctx.message.created_at)
          await ctx.reply(embed=hacker1)
        else:
          await member.timeout(discord.utils.utcnow() + t, reason="Command Used By: {0}".format(ctx.author))
          await ctx.send(embed=hacker)
      except:
        print("an error occured")


    @commands.hybrid_command(name="unmute", description="Unmutes a member .",usage="unmute <member>")
    @blacklist_check()   
    @ignore_check()
    @commands.cooldown(1, 20, commands.BucketType.member)
    @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    ...
    async def untimeout(self, ctx, member: discord.Member):
      if member.is_timed_out():
        try:
          ...
          hacker5 = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Successfully Unmuted {member.name}", timestamp=ctx.message.created_at)
          await ctx.reply(embed=hacker5) 
        except Exception as e:
          hacker = discord.Embed(color=0x01f5b6,description="<a:error:1002226340516331571> | Unable to Remove Timeout:\n```py\n{}```".format(e), timestamp=ctx.message.created_at) 
          await ctx.send(embed=hacker)
      else:
        hacker1 = discord.Embed(color=0x01f5b6,description="<a:error:1002226340516331571> | {} Is Not Muted".format(member.mention), timestamp=ctx.message.created_at)
    ...
    async def _kick(self, ctx: commands.Context, member: discord.Member, *, reason=None):
        ...
        if ctx.author.top_role.position > member.top_role.position or member == ctx.guild.owner:
            ...
            hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | {member.display_name} has been kicked from this guild, for: {reason}", timestamp=ctx.message.created_at)
            ...
            hacker.set_thumbnail(url =f"{ctx.author.avatar}")
            await ctx.send(embed=hacker)
            hacker1 = discord.Embed(color=0x01f5b6,description=f":exclamation: | You have been kicked from {ctx.guild.name} for: {reason}!", timestamp=ctx.message.created_at)
    ...
    async def _warn(self, ctx: commands.Context, member: discord.Member, * , reason="No Reason Provided!"):
        hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | {member.display_name} has been warned for: {reason}", timestamp=ctx.message.created_at)
        #hacker.set_footer(text=f"Made With 💖 By ~ Hacker_xD#0001",icon_url= "https://media.discordapp.net/attachments/1004709672588161044/1005073754889654343/a_9a2d97cca8cf934ac4b3624051ed9baf.gif")
        hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        hacker.set_thumbnail(url =f"{ctx.author.avatar}")
        await ctx.send(embed=hacker)
    ...
    async def _ban(self, ctx: commands.Context, member: discord.Member, *, reason=None):
        ...
        if ctx.author.top_role.position > member.top_role.position or ctx.author == ctx.guild.owner:
            await member.ban(reason=reason)
            hacker = discord.Embed(color=0x00FFE4,description=f"<:GreenTick:1029990379623292938> | {member.display_name} has been successfully banned for the reason: `{reason}`", timestamp=ctx.message.created_at)
            hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
            hacker.set_thumbnail(url =f"{ctx.author.avatar}")
            await ctx.send(embed=hacker)
    ...
    async def _unban(self, ctx: commands.Context, id: int):
        ...
        hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | {user.name} has been successfully unbanned", timestamp=ctx.message.created_at)
        #hacker.set_footer(text=f"Made With 💖 By ~ Hacker_xD#0001",icon_url= "https://media.discordapp.net/attachments/1004709672588161044/1005073754889654343/a_9a2d97cca8cf934ac4b3624051ed9baf.gif")
        hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        hacker.set_thumbnail(url =f"{ctx.author.avatar}")
        await ctx.send(embed=hacker)
    ...
    async def clone(self, ctx: commands.Context, channel: discord.TextChannel):
        await channel.clone()
        hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | {channel.name} has been successfully cloned", timestamp=ctx.message.created_at)
        #hacker.set_footer(text=f"Made With 💖 By ~ Hacker_xD#0001",icon_url= "https://media.discordapp.net/attachments/1004709672588161044/1005073754889654343/a_9a2d97cca8cf934ac4b3624051ed9baf.gif")
        hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        hacker.set_thumbnail(url =f"{ctx.author.avatar}")
        await ctx.send(embed=hacker)
    ...
    async def changenickname(self, ctx: commands.Context, member: discord.Member, * , nick):
        ...
        hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Successfully changed nickname of {member.name}", timestamp=ctx.message.created_at)
        hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
        hacker.set_thumbnail(url =f"{ctx.author.avatar}")
        await ctx.send(embed=hacker)
    ...
    async def do_removal(self, ctx, limit, predicate, *, before=None, after=None, message=True):
        if limit > 1000:
            em = discord.Embed(description=f" Too many messages to search given ({limit}/2000)", color=0x01f5b6)
        ...
        try:
            ...
            em = discord.Embed(description=f" Try a smaller search?", color=0x01f5b6)
            return await ctx.send(embed=em)

        deleted = len(deleted)
        if message is True:
            await ctx.message.delete()
            await ctx.send(embed= discord.Embed(description=f" Successfully removed {deleted} message{'' if deleted == 1 else 's'}.", color=0x01f5b6, delete_after=3))
    ...
    async def _reactions(self, ctx, search=1000):
        ...
        await ctx.message.delete()
        await ctx.send(embed=discord.Embed(description=f"<:GreenTick:1029990379623292938> | Successfully Removed {total_reactions}.", color=0x01f5b6))
...
#
    ...
    async def emojisearch(self, ctx: commands.Context, name: Lower = None):
        if not name:
            ctx.command.reset_cooldown(ctx)
            return await ctx.reply(embed=discord.Embed(description="Please enter a emoji!\n\nExample: `emojisearch cat`",color=0x01f5b6))
        emojis = [str(emoji) for emoji in self.bot.emojis if name in emoji.name.lower() and emoji.is_usable()]
        if len(emojis) == 0:
            ctx.command.reset_cooldown(ctx)
            return await ctx.reply(embed=discord.Embed(description=f"Couldn't find any results for `{name}`, please try again .",color=0x01f5b6))
        paginator = commands.Paginator(prefix="", suffix="", max_size=500)
        for emoji in emojis:
            paginator.add_line(emoji)
        await ctx.reply(embed=discord.Embed(description=f"Found `{len(emojis)}` emojis .",color=0x01f5b6))
    ...
    async def stickersearch(self, ctx: commands.Context, name: Lower = None):
        ...
        if len(stickers) == 0:
            ctx.command.reset_cooldown(ctx)
            return await ctx.reply(embed=discord.Embed(description=f"Couldn't find any results for `{name}` | Try Again .",color=0x01f5b6))
        embeds = []
        ...
        await ctx.reply(embed=discord.Embed(description=f"Found `{len(embeds)}` stickers .",color=0x01f5b6))
        if len(embeds) == 1:
    ...
    async def sticker(self, ctx: commands.Context):
        ...
        if len(stickers) == 0:
            ...
            return await ctx.reply(embed=discord.Embed(
                f" No Stickers!",
                "There are no stickers in this message.",color=0x01f5b6
            ))
        embeds = []
        for sticker in stickers:
...
**Description:** {pack.description}
    ...
    async def temp(self,ctx,role:discord.Role,time,*,user:discord.Member):
        ...
        if role == ctx.author.top_role:
                embed = discord.Embed(
                        description=f"<:error:1018174714750976030> | {role} has the same position as your top role!",
                        color=0x01f5b6
              )
          ...
          if role.position >= ctx.guild.me.top_role.position:
                embed1 = discord.Embed(
                        description=f"<:error:1018174714750976030> | {role} is higher than my role, move my role above {role}.",
                        color=0x01f5b6
              )
        ...
        hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | Successfully added {role.mention} to {user.mention} .",
                      color=0x01f5b6
              )
        await ctx.send(embed=hacker)
    ...
    async def remove(self,ctx,user:discord.Member,role:discord.Role):
        ...
        await user.remove_roles(role)
        hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | Successfully removed {role.mention} from {user.mention} .",
                      color=0x01f5b6
              )
        await ctx.send(embed=hacker)
    ...
    async def delete(self,ctx,role:discord.Role):
        ...
        if role is None:
         embed2 = discord.Embed(
                        description=f"<:error:1018174714750976030> | No role named {role} found in this server .",
                        color=0x01f5b6
              )
         return await ctx.send(embed=embed2)
        await role.delete()
        hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | Successfully deleted {role}",
                      color=0x01f5b6
              )
        await ctx.send(embed=hacker)
    ...
    async def create(self,ctx,*,name):
        ...
        if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
            hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | Successfully created role with the name {name}",
                      color=0x01f5b6
              ) 
    ...
    async def rename(self,ctx,role:discord.Role,*,newname):
        ...
        if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
            ...
            await ctx.send(f"<:GreenTick:1018174649198202990> | Role {role.name} has been renamed to {newname}")
        elif role is None:
            embed2 = discord.Embed(
                        description=f"<:error:1018174714750976030> | No role named {role} found in this server .",
                        color=0x01f5b6
