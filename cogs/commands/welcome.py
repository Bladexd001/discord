...
class Welcomer(commands.Cog):
    ...
    async def _ar_config(self, ctx):
        if data := getDB(ctx.guild.id):
            ...
            emb = discord.Embed(color=0x01f5b6, title=f"Autorole of - {ctx.guild.name}").add_field(name="__Humans__", value=hums, inline=False).add_field(name="__Bots__", value=bos, inline=False)
    ...
    @_autorole_reset.command(name="humans", help="Clear autorole config for the server .")
    ...
    async def _autorole_humans_reset(self, ctx):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if rl == []:
                embed = discord.Embed(
                        description=f"<:error:1018174714750976030> | This server don't have any autoroles setupped .",
                        color=0x01f5b6
              )
                ...
                if rl != []:
                    ...
                    hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | Succesfully cleared all human autoroles for {ctx.guild.name} .",
                      color=0x01f5b6
              )
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if rl == []:
                ...
                if rl != []:
                    ...
                    updateDB(ctx.guild.id, data)
                    hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | Succesfully cleared all bot autoroles for this server .",
                      color=0x01f5b6
                   )
                    await ctx.send(embed=hacker)
        else:
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(brl) == 0 and len(hrl) == 0:
                ...
                if hrl != []:
                    ...
                    updateDB(ctx.guild.id, data)
                    hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | Succesfully cleared all autoroles for this server .",
                      color=0x01f5b6
                   )
                    await ctx.send(embed=hacker)
        else:
    ...
    @_autorole_humans.command(name="add", help="Add role to list of autorole humans users.")
    @blacklist_check()   
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    ...
    async def _autorole_humans_add(self, ctx, role: discord.Role):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(rl) == 5:
                embed = discord.Embed(
                        description=f"<:error:1018174714750976030> | You have reached maximum channel limit for autorole humans which is five .",
                        color=0x01f5b6
              )
                await ctx.send(embed=embed)
            else:
                if str(role.id) in rl:
                    embed1 = discord.Embed(
                        description="<:error:1018174714750976030> | {} is already in human autoroles .".format(role.mention),
                        color=0x01f5b6
              )
                    ...
                    updateDB(ctx.guild.id, data)
                    hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | {role.mention} has been added to human autoroles .",
                      color=0x01f5b6
                   )
                    await ctx.send(embed=hacker)
        else:
    ...
    @_autorole_humans.command(name="remove", help="Remove a role from autoroles for human users.")
    @blacklist_check()   
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    ...
    async def _autorole_humans_remove(self, ctx, role: discord.Role):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(rl) == 0:
                embed = discord.Embed(
                        description=f"<:error:1018174714750976030> | This server dont have any autrole humans setupped yet .",
                        color=0x01f5b6
              )
                await ctx.send(embed=embed)
            else:
                if str(role.id) not in rl:
                    ...
                    updateDB(ctx.guild.id, data)
                    hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | {role.mention} has been removed from human autoroles .",
                      color=0x01f5b6
                   )
                    await ctx.send(embed=hacker)
        else:
    ...
    @_autorole_bots.command(name="add", help="Add role to list of autorole bot users.")
    @blacklist_check()   
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    ...
    async def _autorole_bots_add(self, ctx, role: discord.Role):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(rl) == 5:
                embed = discord.Embed(
                        description=f"<:error:1018174714750976030> | You have reached maximum role limit for autorole bots which is five.",
                        color=0x01f5b6
              )
                await ctx.send(embed=embed)
            else:
                if str(role.id) in rl:
                    embed1 = discord.Embed(
                        description="<:error:1018174714750976030> | {} is already in bot autoroles.".format(role.mention),
                        color=0x01f5b6
              )
                    await ctx.send(embed=embed1)
                else:
                    ...
                    updateDB(ctx.guild.id, data)
                    hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | {role.mention} has been added to bot autoroles .",
                      color=0x01f5b6
                   )
                    await ctx.send(embed=hacker)
        else:
    ...
    @_autorole_bots.command(name="remove", help="Remove a role from autoroles for bot users.")
    @blacklist_check()   
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    ...
    async def _autorole_bots_remove(self, ctx, role: discord.Role):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(rl) == 0:
                ...
                if str(role.id) not in rl:
                    embed1 = discord.Embed(
                        description="<:error:1018174714750976030> | {} is not in bot autoroles.".format(role.mention),
                        color=0x01f5b6
              )
                    await ctx.send(embed=embed1)
                else:
                    ...
                    updateDB(ctx.guild.id, data)
                    hacker = discord.Embed(
                      description=f"<:GreenTick:1018174649198202990> | {role.mention} has been removed from bot autoroles.",
                      color=0x01f5b6
                   )
                    await ctx.send(embed=hacker)
        else:
    ...
    async def _greet_thumbnail(self, ctx, thumbnail_link):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if streamables.search(thumbnail_link):
              ...
              updateDB(ctx.guild.id, data)
              hacker = discord.Embed(color=0x01f5b6,description="<:GreenTick:1029990379623292938> | Successfully updated the welcome thumbnail url .", timestamp=ctx.message.created_at)
    ...
    async def _greet_image(self, ctx, *, image_link):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if streamables.search(image_link):
                ...
                updateDB(ctx.guild.id, data)
                hacker = discord.Embed(color=0x01f5b6,description="<:GreenTick:1029990379623292938> | Successfully updated the welcome image url .", timestamp=ctx.message.created_at)
                hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=hacker)
            else:
                await ctx.send("Oops, Kindly put a valid link.")
...
```""",color=0x01f5b6)
    ...
    async def _greet_autodel(self, ctx, *, autodelete_second):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            ...
            updateDB(ctx.guild.id, data)
            hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Successfully updated the welcome autodelete second to {autodelete_second} .\nFrom now welcome message will be deleted after {autodelete_second} .", timestamp=ctx.message.created_at)
            hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
            await ctx.send(embed=hacker)
        else:
          hacker5 = discord.Embed(description="""```diff
    ...
    async def _greet_message(self, ctx: commands.Context):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            ...
            try:
                ...
                updateDB(ctx.guild.id, data)
                hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Successfully updated the welcome message .", timestamp=ctx.message.created_at)
                hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=hacker)
        else:
          hacker5 = discord.Embed(description="""```diff
    ...
    async def _greet_embed(self, ctx):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if data["welcome"]["embed"] == True:
                ...
                updateDB(ctx.guild.id, data)
                hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Okay, Now your embed is removed and welcome message will be a plain message .", timestamp=ctx.message.created_at)
                hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=hacker)
            elif data["welcome"]["embed"] == False:
                data["welcome"]["embed"] = True
                updateDB(ctx.guild.id, data)
                hacker1 = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Okay, Now your embed is enabled and welcome message will be a embed message.", timestamp=ctx.message.created_at)
    ...
    async def _greet_ping(self, ctx):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if data["welcome"]["ping"] == True:
                ...
                updateDB(ctx.guild.id, data)
                hacker = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Okay, Now your embed ping is disabled and users won't get pinged upon welcome .", timestamp=ctx.message.created_at)
                hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=hacker)
            elif data["welcome"]["ping"] == False:
                data["welcome"]["ping"] = True
                updateDB(ctx.guild.id, data)
                hacker1 = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Okay, Now your embed ping is enabled and I will ping new users outside the embed .", timestamp=ctx.message.created_at)
                hacker1.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}") 
                await ctx.send(embed=hacker1)
        else:
          hacker5 = discord.Embed(description="""```diff
...
```""",color=0x01f5b6)
    ...
    @_greet_channel.command(name="add", help="Add a channel to the welcome channels list.")
    @blacklist_check()   
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    ...
    async def _greet_channel_add(self, ctx, channel: discord.TextChannel):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(chh) == 3:
                hacker = discord.Embed(color=0x01f5b6,description=f"<a:error:1002226340516331571> | You have reached maximum channel limit for channel which is one .", timestamp=ctx.message.created_at)
                hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=hacker)
            else:
                if str(channel.id) in chh:
                    hacker1 = discord.Embed(color=0x01f5b6,description=f"<a:error:1002226340516331571> | This channel is already in the welcome channels list .", timestamp=ctx.message.created_at)
                    hacker1.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
                    await ctx.send(embed=hacker1)
                else:
                    chh.append(str(channel.id))
                    updateDB(ctx.guild.id, data)
                    hacker4 = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Successfully added {channel.mention} to welcome channel list .", timestamp=ctx.message.created_at)
...
```""",color=0x01f5b6)
    ...
    @_greet_channel.command(name="remove", help="Remove a chanel from welcome channels list .")
    @blacklist_check()   
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    ...
    async def _greet_channel_remove(self, ctx, channel: discord.TextChannel):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            if len(chh) == 0:
                hacker = discord.Embed(color=0x01f5b6,description=f"<a:error:1002226340516331571> | This server dont have any welcome channel setupped yet .", timestamp=ctx.message.created_at)
                hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
                await ctx.send(embed=hacker)
            else:
                if str(channel.id) not in chh:
                    hacker1 = discord.Embed(color=0x01f5b6,description=f"<a:error:1002226340516331571> | This channel is not in the welcome channels list .", timestamp=ctx.message.created_at)
                    hacker1.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
                    await ctx.send(embed=hacker1)
                else:
                    chh.remove(str(channel.id))
                    updateDB(ctx.guild.id, data)
                    hacker3 = discord.Embed(color=0x01f5b6,description=f"<:GreenTick:1029990379623292938> | Successfully removed {channel.mention} from welcome channel list .", timestamp=ctx.message.created_at)
...
```""",color=0x01f5b6)
    ...
    async def welctestt(self, ctx):
        ...
        user = ctx.author
        if chan == []:
            hacker = discord.Embed(color=0x01f5b6,description=f"<a:error:1002226340516331571> | Oops, Kindly setup your welcome channel first .", timestamp=ctx.message.created_at)
            hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
            await ctx.send(embed=hacker)
        else:
            ...
            em.set_author(name=ctx.author.name, icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
