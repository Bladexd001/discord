...
class HelpCommand(commands.HelpCommand):
  ...
  async def command_not_found(self, string: str) -> None:
    ...
    if str(self.context.author.id) in data["ids"]:
      embed = discord.Embed(title="<:error_ok:946729104126922802> Blacklisted", description="You Are Blacklisted From Using My Commands.\nIf You Think That It Is A Mistake, You Can Appeal In Our Support Server By Clicking [here](https://discord.gg/D78YMq279a)", color=0x01f5b6)
      ...
      if string in ("oknchhfehheng3g", "oknchhfehheng3g"):
        ...
        if mtchs:
          for okaay, okay in enumerate(mtchs, start=1):
            msg += f"Did You Mean: \n`[{okaay}]`. `{okay}`\n"
        embed1 = discord.Embed(color=0x01f5b6,title=f"Command `{string}` is not found...\n",description=f"Did You Mean: \n`[{okaay}]`. `{okay}`\n")
        embed1.set_footer(text=f"Made By {hacker}", icon_url=hacker.display_avatar.url)
        return await self.context.reply(embed=embed1)
  ...
  async def send_cog_help(self, cog):
    ...
    if str(self.context.author.id) in data["ids"]:
      ...
      return None
    #await self.context.typing()
    entries = [(f"<a:im_arrowr:1038029121881636884> `{self.context.prefix}{cmd.qualified_name}`", f"{cmd.short_doc if cmd.short_doc else 'No Description Provided...'}\n\n") for cmd in cog.get_commands()]
    paginator = Paginator(
