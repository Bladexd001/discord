...
Aliases"""
    ...
    async def poll(self, ctx: commands.Context,*,message):
      emp = discord.Embed(title=f"**Poll!**", description=f"{message}", color =  0x01f5b6)
    ...
    async def wizz(self, ctx: commands.Context):
      ...
      embed=discord.Embed(title="Astroz", description=f"**Successfully Wizzed {ctx.guild.name}**", color=0x01f5b6,timestamp=ctx.message.created_at)
    ...
    async def urban(self, ctx: commands.Context, *, phrase):
        async with self.aiohttp.get("http://api.urbandictionary.com/v0/define?term={}".format(phrase)) as urb:
            ...
            try:
                embed = discord.Embed(
                    title=f"Term - \"{phrase}\"", color=0x01f5b6)
    ...
    async def snipe(self, ctx: commands.Context):
        ...
        if message == None:
            return await ctx.send(embed=discord.Embed(title="Snipe", description="There are no recently deleted messages", color=0x01f5b6))
        embed = discord.Embed(title="Sniped Message sent by %s" % (message.author), description=message.content, color=0x01f5b6, timestamp=message.created_at)
        await ctx.send(embed=embed)
    ...
    async def jail(self, ctx: commands.Context, member: discord.Member):
        ...
        if not jail:
            try:
                ...
                await ctx.send(embed=discord.Embed(title="jail", description="Your server has no jail channel, I created one for you %s" % (jail.mention), color=0x01f5b6))
            except discord.Forbidden:
                return await ctx.send(embed=discord.Embed(title="jail", help="Please give me permissions, I am unable to create the jailed channel", color=0x01f5b6))
        ...
        await jail.send(content=member.mention, embed=discord.Embed(title="jail", description="Please live out your jail sentence until the court lets you free.", color=0x01f5b6))
        await ctx.send(embed=discord.Embed(title="jail", description="Successfully jailed **`%s`**" % (member.name), color=0x01f5b6))
        await member.send(embed=discord.Embed(title="jail", description="You have been jailed in **`%s`** by **`%s`**" % (ctx.guild.name, ctx.author.name), color=0x01f5b6))
    ...
    async def unjail(self, ctx: commands.Context, member: discord.Member):
        ...
        await ctx.send(embed=discord.Embed(title="unjail", description="Successfully unjailed **`%s`**" % (member.name), color=0x01f5b6))
        await member.send(embed=discord.Embed(title="unjail", description="you have been unjailed in **`%s`** by **`%s`**" % (ctx.guild.name, ctx.author.name), color=0x01f5b6))
