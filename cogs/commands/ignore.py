...
class Ignore(commands.Cog):
    ...
    @_ignore.group(name="channel", aliases=["chnl"],  invoke_without_command=True)
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    @blacklist_check()
    @commands.guild_only()
      ...
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
        with open('ignore.json', 'r') as ignore:
          ...
          if str(channel.id) in ignores["ids"]:
            embed = discord.Embed(title="Error!", description=f"{channel.mention} is already in ignore channel list .", color=0x01f5b6)
            await ctx.reply(embed=embed, mention_author=False)
          else:
            add_channel_to_ignore(channel.id)
            embed = discord.Embed(description=f"Now onwards {channel.mention} will be ignored by the bot.", color=0x01f5b6)
            await ctx.reply(embed=embed, mention_author=False)
      else:
    ...
    async def channel_remove(self, ctx, channel: discord.TextChannel):  
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
        ...
        embed = discord.Embed(
                description=f"<:GreenTick:1029990379623292938> | {channel.mention} has been successfully removed from ignore channel list",
                color=0x01f5b6
            )
