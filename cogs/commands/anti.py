...
class Security(Cog):
    ...
    for channel in ctx.guild.channels:
        if channel.name in ('rules', 'moderator-only'):
            try:
                ...
                pass
    hacker5 = discord.Embed(title="Astroz Security", description="<:GreenTick:1029990379623292938> | Successfully Deleted All Channels With Name Of `rules` and `moderator-only`",color=0x01f5b6)
  ...
  async def punishment_set(self, ctx, punishment: str):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
            ...
            if kickOrBan == "kick":
                ...
                hacker = discord.Embed(title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Changed Punishment To: **{kickOrBan}** For {ctx.guild.name}",color=0x01f5b6)
                ...
                data = getConfig(ctx.guild.id)
                data["punishment"] = "ban"
                hacker1 = discord.Embed(title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Changed Punishment To: **{kickOrBan}** For {ctx.guild.name}",color=0x01f5b6)
                ...
                hacker3 = discord.Embed(title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Changed Punishment To: **{kickOrBan}** For {ctx.guild.name}",color=0x01f5b6)
                await ctx.reply(embed=hacker3, mention_author=False)

                updateConfig(ctx.guild.id, data)
            else:
               hacker6 = discord.Embed(title="Astroz Security", description="Invalid Punishment Type\nValid Punishment Type(s) Are: Kick, Ban, None",color=0x01f5b6)
  ...
  async def punishment_show(self, ctx: Context):
    ...
    hacker5 = discord.Embed(color=0x01f5b6,title="Astroz Security", description="Custom punishment of anti-nuke and automod in this server is: **{}**".format(punish.title()))
    await ctx.reply(embed=hacker5,mention_author=False)
      ...
      for channel in ctx.message.guild.channels:
        if channel.name == channeltodelete:
            try:
                ...
                pass
      hacker1 = discord.Embed(title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Deleted All Channels With The Name Of {channeltodelete}",color=0x01f5b6)         
      await ctx.reply(embed=hacker1, mention_author=False)
      ...
      for channel in ctx.message.guild.channels:
        if channel.name == channeltodelete:
            try:
                ...
                pass
      hacker2 = discord.Embed(title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Deleted All Channels With The Name Of {channeltodelete}",color=0x01f5b6)         
      ...
      for role in ctx.message.guild.roles:
        if role.name == roletodelete:
            try:
                ...
                pass
      hacker = discord.Embed(title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Deleted All Roles With The Name Of {roletodelete}",color=0x01f5b6)         
      await ctx.reply(embed=hacker, mention_author=False)
    elif ctx.author.id == 143853929531179008:
      for role in ctx.message.guild.roles:
        if role.name == roletodelete:
            try:
                ...
                pass
      hacker3 = discord.Embed(title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Deleted All Roles With The Name Of {roletodelete}",color=0x01f5b6)         
      await ctx.reply(embed=hacker3, mention_author=False)
    else:
      hacker5 = discord.Embed(description="""```yaml\n - You must have Administrator permission.\n - Your top role should be above my top role.```""",color=0x01f5b6)
      hacker5.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
  ...
  async def whitelist_add(self, ctx, user: discord.User):
    ...
    if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
      if len(wled) == 15:
        hacker = discord.Embed(title="Astroz Security", description=f"<:error:1018174714750976030> This server have already maximum number of whitelisted users (15)\nRemove one to add another :)",color=0x01f5b6)
        await ctx.reply(embed=hacker, mention_author=False)
      else:
        if str(user.id) in wled:
          hacker1 = discord.Embed(title="Astroz Security", description=f"<:error:1018174714750976030> That user is already in my whitelist.",color=0x01f5b6)          
          await ctx.reply(embed=hacker1, mention_author=False)
        else:
          wled.append(str(user.id))
          updateConfig(ctx.guild.id, data)
          hacker4 = discord.Embed(color=0x01f5b6,title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Whitelisted {user.mention} For {ctx.guild.name}")
  ...
  async def whitelist_remove(self, ctx, user: discord.User):
    ...
    if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
      if str(user.id) in wled:
        ...
        updateConfig(ctx.guild.id, data)
        hacker = discord.Embed(color=0x01f5b6,title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Unwhitelisted {user.mention} For {ctx.guild.name}")      
        await ctx.reply(embed=hacker, mention_author=False)
      else:
        hacker2 = discord.Embed(color=0x01f5b6,title="Astroz Security", description="<:error:1018174714750976030> | That user is not in my whitelist.")  
        await ctx.reply(embed=hacker2, mention_author=False)
    else:
      hacker5 = discord.Embed(description="""```yaml\n - You must have Administrator permission.\n - Your top role should be above my top role.```""",color=0x01f5b6)
      hacker5.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
  ...
  async def whitelist_show(self, ctx):
      ...
      wled = data["whitelisted"]
      if len(wled) == 0:
        hacker = discord.Embed(color=0x01f5b6,title="Astroz Security", description=f"<:error:1018174714750976030> | There aren\'t any whitelised users for this server")
        await ctx.reply(embed=hacker, mention_author=False)
      else:
          entries = [f"`{no}` | <@!{idk}> | ID: [{idk}](https://discord.com/users/{idk})" for no, idk in enumerate(wled, start=1)]
          paginator = Paginator(
  ...
  async def wl_reset(self, ctx: Context):
    ...
    if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
      ...
      data["whitelisted"] = []
      updateConfig(ctx.guild.id, data)
      hacker = discord.Embed(color=0x01f5b6,title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | Successfully Cleared Whitelist Database For **{ctx.guild.name}**")         
      await ctx.reply(embed=hacker, mention_author=False)
    else:
      hacker5 = discord.Embed(description="""```yaml\n - You must have Administrator permission.\n - Your top role should be above my top role.```""",color=0x01f5b6)
      hacker5.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
  ...
  @_whitelist.command(name="role", help="Add a role to whitelisted role",usage="Antinuke whitelist role")
  @blacklist_check()      
  @ignore_check()
  ...
  async def whitelist_role(self, ctx, role: discord.Role):
    ...
    if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
        updateConfig(ctx.guild.id, data)
        hacker4 = discord.Embed(color=0x01f5b6,title="Astroz Security", description=f"<:GreenTick:1029990379623292938> | {role.mention} Has Been Added To Whitelisted Role For {ctx.guild.name}")
        await ctx.reply(embed=hacker4, mention_author=False)
