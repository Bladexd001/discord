...
class Astroz(commands.AutoShardedBot):
    def __init__(self, *arg, **kwargs):
      self.topgg_headers = {"Authorization": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjEwMTI2MjcwODgyMzIxNjUzNzYiLCJib3QiOnRydWUsImlhdCI6MTY3MDU4MzE3NH0.WULUKASz45RZduUMpTCqzHt0nPk3MqnpeJHF3YNgBo8"}      
    ...
    async def on_connect(self):
      await self.change_presence(status=discord.Status.dnd, activity=discord.Activity(type=discord.ActivityType.listening, name='@Help | Astroz Development '))
      async with aiohttp.ClientSession(headers=self.topgg_headers)as session:
        async with session.post("https://top.gg/api/bots/1012627088232165376/stats", json={"server_count": len(self.guilds), "shard_count": len(self.shards)}) as r:
