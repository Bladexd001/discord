...
class FieldPagePaginator(menus.ListPageSource):
    ...
    async def format_page(self,menu: HackerPaginator, entries: list[tuple[Any, Any]]) -> discord.Embed:
        ...
        if maximum > 1:
            text = f'Astroz • Page {menu.current_page + 1}/{maximum}'
            self.embed.set_footer(text=text, icon_url="https://cdn.discordapp.com/avatars/1012627088232165376/7ed46a33af87a6927f4934031ff13a05.webp?size=2048")
...
class TextPaginator(menus.ListPageSource):
    ...
    async def format_page(self, menu, content):
        maximum = self.get_max_pages()
        if maximum > 1:
            return f'{content}\nAstroz • Page {menu.current_page + 1}/{maximum}'
        return content
