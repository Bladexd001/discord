...
class InvocationFeature(Feature):
    ...
    async def jsk_repeat(self, ctx: ContextT, times: int, *, command_string: str):
        ...
        with self.submit(ctx):  # allow repeats to be cancelled
            for _ in range(times):
                ...
                if alt_ctx.command is None:
                    hacker5 = discord.Embed(description=f'Command "{alt_ctx.invoked_with}" is not found',color=0x01f5b6)
                    hacker5.set_footer(text="Made By ~ Hacker_xD#0007",icon_url=self.bot.user.display_avatar.url)
    ...
    async def jsk_debug(self, ctx: ContextT, *, command_string: str):
        ...
        hacker5 = discord.Embed(description=f"Command `{alt_ctx.command.qualified_name}` finished in {end - start:.3f}s.",color=0x01f5b6)
        hacker5.set_footer(text="Made By ~ Hacker_xD#0007",icon_url=self.bot.user.display_avatar.url)
        return await ctx.send(embed=hacker5)
